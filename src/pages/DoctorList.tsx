...
const DoctorList = () => {
  ...
  const filtered = useMemo(() => {
    return doctors.filter((doc) => {
      ...
      const matchFee = maxFee === "all" || (maxFee === "100" && doc.fee <= 100) || (maxFee === "150" && doc.fee <= 150) || (maxFee === "200" && doc.fee <= 200);
      return matchSearch && matchSpecialty && matchGender && matchFee;
    });
  }, [doctors, search, selectedSpecialties, gender, maxFee]);
      ...
      <div className="container py-8">
        <div className="flex gap-8">
                ...
                <div className="space-y-2 max-h-48 overflow-y-auto">
                  {specialties.map((spec) => (
                    <label key={spec} className="flex items-center gap-2 text-sm cursor-pointer">
                      <Checkbox checked={selectedSpecialties.includes(spec)} onCheckedChange={() => toggleSpecialty(spec)} />
          ...
          <div className="flex-1 space-y-4">
            ...
            {loading ? (
              <div className="py-16 text-center text-muted-foreground">Loading doctors...</div>
            ) : (
              <>
                ...
                {filtered.map((doctor) => {
                  ...
                  return (
                    <div key={doctor._id} className="rounded-lg border bg-card p-5 flex gap-5 hover:shadow-md transition-shadow">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-muted-foreground mt-2">
                          <span className="flex items-center gap-1"><Star className="h-3.5 w-3.5 fill-warning text-warning" />{doctor.rating} ({doctor.reviews})</span>
                ...
                {filtered.length === 0 && (
                  <div className="py-16 text-center text-muted-foreground">No doctors found matching your criteria.</div>
                )}
              </>
            )}
          </div>
